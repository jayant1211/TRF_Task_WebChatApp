...
$total_chats = "SELECT * from users_chat where (sender_username='$user_name' AND receiver_username='$temp_username') OR (sender_username='$temp_username' AND receiver_username='$user_name') ORDER by 1 ASC";
...
while($row_chats=mysqli_fetch_array($run_chats))
{
    ...
    if($sender_username==$temp_username AND $receiver_username=$user_name)
    {
    echo
    "
    <li class='chat-left'>
    ...
    else if($sender_username==$user_name AND $receiver_username=$temp_username)
    {
        echo"
        <li class='chat-right'>
        <div class='chat-hour'>$msg_date <span class='fa fa-check-circle'></span></div>
